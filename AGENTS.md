...
yarn lint             # Lint all (oxfmt + oxlint)
...
- **Testing**: vitest
- **Linting**: oxlint with `@ffflorian/oxlint-config`
- **Formatting**: oxfmt with `@ffflorian/oxfmt-config`
- **Git hooks**: lefthook (`lefthook.yml`) — runs oxfmt, and oxlint with auto-fix on staged files before commit
...
- oxfmt config: `@ffflorian/oxfmt-config`
...
This runs oxfmt (formatting) and oxlint (linting) with auto-fix across the codebase.
