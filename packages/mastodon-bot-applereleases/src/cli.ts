...
import {AppleReleasesClient} from './AppleReleasesClient';
...
new AppleReleasesClient({accessToken: token, baseURL: server, configFile: config, dryRun: dryRun, forceSending: force})
  .run()
  .then(() => process.exit())
  .catch(error => {
    console.error(error);
